...
from app.services.admin_service import create_classroom, create_teachers_batch, generate_students_for_class

router = APIRouter(prefix="/admin", tags=["admin"])


@router.get("/teachers")
def get_teachers(db: Session = Depends(get_db), admin=Depends(require_admin)):
...
def get_classes(db: Session = Depends(get_db), admin=Depends(require_admin)):
...
def get_class_students(class_id: int, db: Session = Depends(get_db), admin=Depends(require_admin)):
...
@router.get("/students")
def get_students(db: Session = Depends(get_db), admin=Depends(require_admin)):
